...
def load_form_schema():
    """Load form schema from current_event_form.json or use default fallback."""
    form_file = "current_event_form.json"
    ...
    if os.path.exists(form_file):
        try:
            ...
            st.info("Using default form. Run app_copy_2.py to generate a fresh event form.")
            return get_default_form()
    else:
        st.info(f"📝 No form found. Run app_copy_2.py first to generate current_event_form.json")
        return get_default_form()
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("� Automatic Updates: Run app_copy_2.py to generate a new event form. This app will automatically load it from current_event_form.json")
